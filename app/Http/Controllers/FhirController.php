...
class FhirController extends Controller
{
    ...
    public function read(string $resourceType, string $id): JsonResponse
    {
        ...
        if (!$t) return $this->responseFactory->notFound($resourceType, $id);
        $model = $t->findById($id);
        if (!$model) return $this->responseFactory->notFound($resourceType, $id);
    ...
    {
        $t = $this->resolve($resourceType);
        if (!$t) return $this->responseFactory->operationOutcome([['severity'=>'error','code'=>'not-found','details'=>['text'=>"Unsupported: {$resourceType}"]]], 404);
    ...
    {
        $t = $this->resolve($resourceType);
        if (!$t) return $this->responseFactory->notFound($resourceType, '');
        $fhir = $request->json()->all();
        $v = $this->validator->validate($resourceType, $fhir);
        if (!$v['valid']) return $this->responseFactory->validationError($v['errors']);
        $bv = $t->validateBusinessRules($fhir);
        if (!empty($bv)) return $this->responseFactory->validationError($bv);
